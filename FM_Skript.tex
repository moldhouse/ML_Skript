  ...
  \section{Regression}
    ...
    \subsection{Ordinary Least Squares (OLS)}
      ...
      \begin{enumerate}
        ...
        \begin{equation*}
          X^T = (U \cdot \Lambda \cdot V^T)^T = (V^T)^T\cdot\Lambda^T\cdot U^T = V\cdot\Lambda\cdotU^T
        \end{equation*}
        \begin{equation*}
        ...
        \begin{equation*}
          \begin{align*}
            S^{-1}\cdot X^T &=V\cdot\Lambda^{-2}\cdot\underbrace{V^T\cdotV}_{\mathbbm{1}}
      ...
      unknown:
      \begin{equation*}
        \mu(j_a, j_b) \iff B_j \quad \text{with} j = j_a + D_a \cdot j_b \quad
        \text{'flattening of image'}
      \end{equation*}
      \paragraph{Step 4} discretize Integral: relax $\delta((a,b) \in ray)$ \\
      $\Rightarrow$ triangular shape, change integral into sum:
      \begin{equation*}
        \begin{align*}
    ...
    \subsection{If both Y and X are noisy - Total Least Squares (TLS)} $\hat{=}$
      ...
      \begin{itemize}
        \item standard interpretation of OLS:
        \begin{itemize}
          \item eq. $X\beta=Y$ has no solution \\
          $\Rightarrow$ minimize sq difference between LHS ans RHS $(X\beta-Y)^2$
        \end{itemize}
