...
\begin{document}
  ...
  \section{Regression}
    ...
    \begin{itemize}
      ...
      OLS can learn parameters $\beta_j$ and $\beta_{j'}$ suc that
      \[\sum_i (X_i\beta_j+X_i\beta_{j'}) = 0\] on the training set but $\beta_j$
      and $\beta_{j'}$ are big numbers that cancel on TS \Rightarrow $\sum_i
      (X_i\beta_j+X_i\beta_{j'})$ on \underline{test set} is huge $\Rightarrow$
      overfitting
      ...
      \item only meaningful if the $\beta_j$ have equal importance/scale to makeroompenalties comparable
